...
For more information about your warn, please read #rules (<#431541621893627914>).`;
      ...
      setTimeout(() => {
        if ((client.muteDB.get(member.id) || 0) < Date.now()) {
          ...
          mutedMember.roles.remove(client.config.mutedRole, `Scheduled unmute after ${mute} minutes.`).catch((err) => {
            client.error(client.channels.cache.get(config.modLog), 'Unmute Failed!', `I've failed to unmute this member! ${err}\nID: ${member.id}`);
          });
        }
      }, mute * 60000);
    } catch (err) {
      client.error(interaction, 'Mute Failed!', `I've failed to mute this member! ${err}`);
    }
  }

  // Notify in channel
  client.success(interaction, 'Damage Inflicted!', `**${member.user.tag || member}** has lost **${newDamage} heart${newDamage === 1 ? '' : 's'}**!`);
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addField('Remaining Hearts', 3 - (curDamage + newDamage), true)
